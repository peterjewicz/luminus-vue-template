...
The `home-routes` handler in the `vue-test.routes.home` namespace
...
The `home-page` function will in turn call the `vue-test.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `vue-test.handler` namespace:
...
Request middleware functions are located under the `vue-test.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `vue-test.dev-middleware` namespace found in
